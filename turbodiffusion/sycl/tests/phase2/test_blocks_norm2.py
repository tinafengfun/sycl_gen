#!/usr/bin/env python3
"""
Phase 2.2 Test: blocks[*].norm2 SYCL Replacement (Fixed)

Tests replacing the norm2 layer (FFN output LayerNorm) in all blocks
...
print("="*70)
print("Phase 2.2: blocks[*].norm2 SYCL Replacement Test (Fixed)")
print("="*70)
...
def main():
    ...
    for block_idx in [0, 15, 29]:
        result = test_layernorm(f'blocks[{block_idx}].norm2', (2, 64, 1536))
        results['tests'].append(result)
    
    # Test multiple blocks (simulated with larger batch)
    print("\n" + "="*70)
    ...
    result = test_layernorm('blocks[10-19].norm2', (20, 64, 1536))
    results['tests'].append(result)
    
    result = test_layernorm('blocks[20-29].norm2', (20, 64, 1536))
    results['tests'].append(result)
    ...
    output_file = os.path.join(output_dir, 'phase2_2_fixed_results.json')
