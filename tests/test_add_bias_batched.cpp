...
// Add Bias Batched Kernel - 3 optimization versions
// V0: Baseline (WG=256, simple loop)
namespace v0 {
inline int DivUp(int a, int b) { return (a + b - 1) / b; }

template <typename T>
void addBiasBatched_kernel(T* output, const T* input, const T* bias, 
                           int N, int C, int spatial, const sycl::nd_item<1> &item) {
...
template <typename T>
void addBiasBatched(T* output, const T* input, const T* bias,
    ...
    queue.parallel_for(
        ...
        [=](sycl::nd_item<1> item) [[sycl::reqd_sub_group_size(16)]] {
            addBiasBatched_kernel(output, input, bias, N, C, spatial, item);
        }
    );
}
}
...
// V2: WG=512 + loop unrolling
namespace v2 {
inline int DivUp(int a, int b) { return (a + b - 1) / b; }

template <typename T>
void addBiasBatched_kernel(T* output, const T* input, const T* bias, 
                           int N, int C, int spatial, int total, const sycl::nd_item<1> &item) {
...
void addBiasBatched(T* output, const T* input, const T* bias,
    ...
    queue.parallel_for(
        sycl::nd_range<1>(blocks * wg_size, wg_size),
        [=](sycl::nd_item<1> item) [[sycl::reqd_sub_group_size(16)]] {
            addBiasBatched_kernel(output, input, bias, N, C, spatial, total, item);
        }
    );
}
}
...
int main() {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "Add Bias Batched Kernel Benchmark" << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    int iterations = 100;
    std::ofstream csv("add_bias_batched_results.csv");
    ...
    for (int N : sizes) {
        ...
        double total_ops = (double)total_size * 1;  // 1 add per element
        ...
        run_test("V0", [&]() { v0::addBiasBatched(d_output, d_input, d_bias, N, C, H, W, queue); });
        run_test("V1", [&]() { v1::addBiasBatched(d_output, d_input, d_bias, N, C, H, W, queue); });
        run_test("V2", [&]() { v2::addBiasBatched(d_output, d_input, d_bias, N, C, H, W, queue); });
    ...
    csv.close();
    std::cout << std::endl << "✅ Add Bias Batched testing completed!" << std::endl;
    return 0;
}
