...
// SE Layer NHWC - Squeeze-and-Excitation layer
// V0: Baseline (two-pass: squeeze + excitation)
namespace v0 {
...
void seLayer_kernel(T* output, const T* input, const T* w1, const T* b1, 
    ...
    for (int c = tid; c < C; c += threads) {
        ...
        val += (float)b2[c];
        float scale = 1.0f / (1.0f + sycl::exp(-val));  // Sigmoid
...
// V1: Smaller WG=64 with aggressive unrolling
namespace v1 {
inline int DivUp(int a, int b) { return (a + b - 1) / b; }
...
void seLayer_kernel(T* output, const T* input, const T* w1, const T* b1, 
    ...
    // Step 2: FC1 + ReLU with unrolling
    for (int k = tid; k < se_K; k += threads) {
        float val = 0.0f;
        #pragma unroll 8
        for (int c = 0; c < C; c++) {
    ...
    // Step 3: FC2 + Sigmoid with unrolling
    for (int c = tid; c < C; c += threads) {
        float val = 0.0f;
        #pragma unroll 8
        for (int k = 0; k < se_K; k++) {
    ...
    // Step 4: Scale input with unrolling
    for (int c = tid; c < C; c += threads) {
        float scale = shared_squeeze[c];
        #pragma unroll 4
        for (int h = 0; h < H; h++) {
...
// V2: Single work-item per batch (no collaboration)
namespace v2 {
inline int DivUp(int a, int b) { return (a + b - 1) / b; }
...
void seLayer_kernel(T* output, const T* input, const T* w1, const T* b1, 
                    const T* w2, const T* b2, int N, int C, int H, int W, int se_K,
                    float epsilon, sycl::item<1> item) {
    int n = item.get_id(0);
    if (n >= N) return;
    
    // Each thread processes entire SE layer for one sample
    ...
    for (int k = 0; k < se_K; k++) {
        ...
        #pragma unroll 8
        for (int c = 0; c < C; c++) {
            val += squeeze[c] * (float)w1[c * se_K + k];
        }
        val += (float)b1[k];
        fc[k] = (val > 0) ? val : 0;
    }
    ...
    for (int c = 0; c < C; c++) {
        ...
        #pragma unroll 8
        for (int k = 0; k < se_K; k++) {
            val += fc[k] * (float)w2[k * C + c];
        }
        val += (float)b2[c];
        scale[c] = 1.0f / (1.0f + sycl::exp(-val));
    }
    ...
    for (int c = 0; c < C; c++) {
        ...
        for (int h = 0; h < H; h++) {
            ...
            for (int w = 0; w < W; w++) {
                int idx = ((n * H + h) * W + w) * C + c;
                output[idx] = (T)((float)input[idx] * scale[c]);
            }
        }
    }
}
...
int main() {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "SE Layer NHWC Kernel Benchmark" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << "GPU: " << device.get_info<sycl::info::device::name>() << std::endl;
    std::cout << std::endl;

    std::vector<int> sizes = {64, 128, 256};
    int iterations = 30;  // Reduced due to complexity
    std::ofstream csv("se_layer_nhwc_results.csv");
    ...
    for (int N : sizes) {
        ...
        // SE layer: squeeze(2 ops) + FC1(C*se_K*2) + FC2(se_K*C*2) + scale(1 op)
        ...
        run_test("V0", [&]() { v0::seLayer(d_output, d_input, d_w1, d_b1, d_w2, d_b2, N, C, H, W, se_K, epsilon, queue); });
        run_test("V1", [&]() { v1::seLayer(d_output, d_input, d_w1, d_b1, d_w2, d_b2, N, C, H, W, se_K, epsilon, queue); });
        run_test("V2", [&]() { v2::seLayer(d_output, d_input, d_w1, d_b1, d_w2, d_b2, N, C, H, W, se_K, epsilon, queue); });
    ...
    csv.close();
    std::cout << std::endl << "✅ SE Layer testing completed!" << std::endl;
    return 0;
}
