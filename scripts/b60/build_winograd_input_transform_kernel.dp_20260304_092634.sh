#!/bin/bash
# SYCL Build Script for BMG/XPU - Generated 20260304_092634
...
echo "Timestamp: 20260304_092634"
...
echo "Compiler: icpx"
echo "Compile Flags: -fsycl -O2 -std=c++17 -fno-sycl-unnamed-lambda -sycl-std=2020 -fhonor-nans -fhonor-infinities -fno-associative-math -fno-approx-func -no-ftz -fsycl-targets=spir64_gen,spir64"
...
icpx -fsycl -O2 -std=c++17 -fno-sycl-unnamed-lambda -sycl-std=2020 -fhonor-nans -fhonor-infinities -fno-associative-math -fno-approx-func -no-ftz -fsycl-targets=spir64_gen,spir64 \
  ...
  2>&1 | tee "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log.step1"
...
icpx -fsycl -fsycl-max-parallel-link-jobs=4 --offload-compress -fsycl-targets=spir64_gen,spir64 -Xs '-device pvc,bmg,dg2,arl-h,mtl-h,lnl-m,ptl-h,ptl-u -options -cl-poison-unsupported-fp64-kernels -options -cl-intel-enable-auto-large-GRF-mode -options -cl-fp32-correctly-rounded-divide-sqrt -options -cl-intel-greater-than-4GB-buffer-required' \
  "/workspace/build_sycl/winograd_input_transform_kernel.dp.o" \
  -o "/workspace/build_sycl/winograd_input_transform_kernel.dp.o.linked" \
  2>&1 | tee "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log.step2"
...
# 合并日志
cat "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log.step1" > "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log"
if [ -f "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log.step2" ]; then
    echo "" >> "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log"
    echo "=== Device Link Step ===" >> "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log"
    cat "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log.step2" >> "/workspace/results/b60/compile_winograd_input_transform_kernel.dp_20260304_092634.log"
fi
...
if [ -f "/workspace/build_sycl/winograd_input_transform_kernel.dp.o" ]; then
    ...
    echo "Output file: /workspace/build_sycl/winograd_input_transform_kernel.dp.o"
    ls -lh "/workspace/build_sycl/winograd_input_transform_kernel.dp.o"
    
    # 检查是否包含BMG设备代码
    echo ""
    echo "Checking device code presence..."
    if objdump -h "/workspace/build_sycl/winograd_input_transform_kernel.dp.o" 2>/dev/null | grep -q ".text"; then
